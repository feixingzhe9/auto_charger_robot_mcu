...
/************************************************
* capture_status : Bit15 - 完成捕获标志
*                 Bit14 - 捕捉到上升沿标志
*                 Bit13 - 溢出错误标志
*                 Bit12 - 保留位
*                 Bit11..0 - 更新事件发生次数
*************************************************/
__IO uint16_t capture_status = 0;
__IO uint16_t capture_rise_val = 0;
__IO uint16_t capture_fall_val = 0;
void timer2_capture_init(uint16_t arr,uint16_t psc)
{
...
//pb8 tim4ch3
*********************************************************************/
void time4_ch1_pwm_init(u16 arr,u16 psc)
{
    /*********使能初始化定时器1时钟*************/
    GPIO_InitTypeDef GPIO_InitStructure;
    TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
    ...
    //配置PB10为复用推挽输出
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;//PE10，对应TIM1_CH2N
...
* duty = 100*CCR3/(ARR+1)
*************************************************/
void pwm_puty_set(uint8_t duty)
{
    uint16_t compare;

    compare = duty*(1985+1)/100;//计算比较值
    TIM4->CCR1 = compare;//更新比较值
}


/*******************************************************
* NAME  : send_remote_data
* PARAM : uint8_t sys_num
*         uint8_t key_num
* RETURN: void
*******************************************************/
void send_remote_data(uint8_t sys_num, uint8_t key_num)
{
    ...
    data = ((~key_num & 0xFF) << 24) + ((key_num & 0xFF) << 16) + ((~sys_num & 0xff) << 8) + sys_num;
