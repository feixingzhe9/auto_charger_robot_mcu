...
//        wdg_feed();
        ...
        id = remote_scan();/* recv infrared data */
        calculate_length(cnt);/* get small laser len data */
        remote_calculate(id);/* get infrared data */
        update_status(id);/* update system status */
        can_protocol_period();/* can recv data process */
        led_indicator(tick);/* system led */
    }
}
